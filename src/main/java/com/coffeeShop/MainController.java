...
public class MainController {
    ...
    public String index(Model m, HttpServletRequest request ){
        ...
        if (currentUser!=null)
            if (isAdmin){
                menu.add(new MenuItem("/admin", "Панель администрарования"));
            }
            else
                {
                ...
                menu.add(new MenuItem("/orders", "Список заказов"));
                menu.add(new MenuItem("/logout", "Выйти из аккаунта"));

        }else
            menu.add(new MenuItem("/login", "Вход"));
    ...
    public String loginProcess(HttpServletRequest request, Model m){
        ...
        if (u.login.equals(_login))
            if (u.encryptedPassword.equals(_encryptedPassword)){
                currentUser=u;
                break;
